...
#define DAP_FW_VER      "0.10"   // Firmware Version
...
static __inline void TIMER_START (uint32_t usec)
{
	...
	systick_clear ();
	systick_set_reload (usec * CPU_CLOCK / 1000000);
...
//   response: pointer to response data
//   return:   number of bytes in response
__attribute__ ((__weak__)) uint32_t DAP_ProcessVendorCommand(uint8_t *request, uint8_t *response)
{
